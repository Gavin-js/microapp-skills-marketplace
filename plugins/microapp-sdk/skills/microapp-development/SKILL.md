...
4. **根据应用类型询问核心信息**：
   ...
   **中奖偏好设置功能说明**：
   - PC端预置"抽奖设置"按钮，点击弹出设置面板
   - **排除黑名单**：添加不能中奖的人员（姓名/手机号），抽奖时自动跳过
   - **指定中奖人**：指定必须中奖的人员（姓名/手机号），优先确保这些人中奖
   - 设置持久化保存，抽奖时自动应用偏好规则
...
**中奖偏好设置功能说明**（默认功能）：
- PC端界面预置"抽奖设置"按钮，点击弹出设置面板
- **设置面板功能**：
  - **排除黑名单**：添加不能中奖的人员，支持按姓名或手机号添加，抽奖时自动跳过这些人员
  - **指定中奖人**：指定必须中奖的人员，支持按姓名或手机号添加，抽奖时优先确保这些人中奖
  - **优先级规则**：指定中奖人 > 随机抽奖 > 排除黑名单
  - **数据持久化**：设置使用 `FsYxtMicroApp.storeAppData()` 保存到服务端，抽奖时自动读取并应用
- **UI 设计要求**：
  - 设置按钮位于奖项管理区域旁边
  - 设置面板采用弹窗形式，支持标签页切换（黑名单/指定中奖人）
  - 支持批量导入（按行输入姓名或手机号）
  - 实时显示已添加的人员列表，支持删除操作
